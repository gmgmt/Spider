...
log_sougou = log.Log("sougou_links_en")
sougou_url = config.sougou_url
li_db_words_en = config.li_db_words_en
...
class Insertsql(object):
    ...
    def find_ones(self, data, proxies=None):
        for x in range(1, 2):
            try:
                data['page'] = str(x)
    ...
    def delete_mongo(self, proxies, num):
        ...
        while True:
            ...
            try:
                # 关键词
                item = li_db_words_en.find_and_modify({'status': 0}, {'$set': {"status": 1}})
                if not item:
                    print('程序完结')
                    sys.exit()
                name = item['_id']
                ...
                for word in ['doc', 'docx', 'pdf', 'ppt', 'pptx', 'rtf']:
                    ...
                    if self.page_num % 70 == 1:
                        self.my_cookies = self.find_cookie()
                        print("获取新的cookies", self.my_cookies['SNUID'], self.page_num, os.getpid())
